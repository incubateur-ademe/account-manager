...
description: Cache Components de Next.js 16 - PPR, directive use cache, cacheLife, cacheTag, updateTag. Reference a lire avant d'activer cacheComponents ou d'ajouter un 'use cache'.
...
`cacheComponents` n'est **pas active** dans `next.config.ts`. Ce document decrit ce qui deviendrait
possible si on l'activait, ce n'est pas une description du comportement actuel.

Deux garde-fous avant d'y toucher, tires de `docs/architecture.md` :

- Un ecran d'ecart ou de plan ne doit **jamais** servir une donnee mise en cache sans que la fraicheur
  soit visible. L'operateur decide de couper des acces sur la foi de ce qu'il lit. Si une page cachee
  affiche un etat perime, la decision est fausse et la trace d'audit dira qu'un humain l'a validee.
- Tout ce qui derive d'un `SyncRun` porte deja sa date. Un cache qui masque « dernier scan il y a X »
  casse la regle 4.4 du document d'architecture.

La reference amont ci-dessous est conservee en anglais volontairement, pour ne pas introduire
d'erreurs de traduction sur des noms d'API.
