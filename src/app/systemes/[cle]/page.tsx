  ...
  return (
    <main className={fr.cx("fr-container", "fr-my-6w")}>
      ...
      <p className={fr.cx("fr-text--sm")}>
        Les capacités communes à tous les systèmes restent sur l'écran Systèmes.
      </p>
      ...
      {configuration === undefined ? (
        ...
        <>
          <p className={fr.cx("fr-text--sm")}>
            Telle qu'elle est résolue, défauts compris, et non telle que le fichier l'écrit. Elle
            s'édite dans la clé <code>connectors.{contrat.key}</code> du fichier{" "}
            <code>config.yaml</code> de la politique.
          </p>
          <pre
      ...
      {avecRevue.length === 0 ? (
        <p className={fr.cx("fr-text--sm")}>
          Aucun compte machine déclaré sur ce système. Un compte constaté se déclare depuis la file
          des comptes isolés, et un jeton que rien ne relève depuis l'écran « Comptes de service ».
        </p>
      ) : (
        <Table
          fixed
